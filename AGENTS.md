...
- Each state has its own PNG set: `public/icon/` (active — green→blue gradient dot), `public/icon-idle/` (purple→blue dot), `public/icon-disabled/` (gray dot) — same filenames (`16/32/48/96/128.png`) in each, and each file must be exactly its nominal pixel size (AMO validation warns on mismatches). Replace the files to change the look (no code change needed).
